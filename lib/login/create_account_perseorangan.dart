...
class CreateAccountPerseoranganPage extends StatefulWidget {
  const CreateAccountPerseoranganPage({Key? key}) : super(key: key);

  @override
  State<CreateAccountPerseoranganPage> createState() =>
      _CreateAccountPerseoranganPageState();
}

class _CreateAccountPerseoranganPageState
    extends State<CreateAccountPerseoranganPage> {
  final _namaPerseoranganController = TextEditingController();
  ...
  String _selectedCategory = "Perseorangan";
  ...
  void initState() {
    ...
    //     context.read<UOCListSearchCubit>().uOCCreateAccount(filter);
    //   }
    // });
  }

  Future<List<UOCListSearchAccesses>> fetchUpdatedDistriks() async {
    // Access the cubit from the widget tree using BlocProvider
    var cubit = context.read<UOCListSearchCubit>();
    String filter = _searchController.text;
    var response =
        await cubit.uOCCreateAccount(filter); // Call the method on the instance
    print("Response from uOCCreateAccount: $response");
    if (response != null && filter.length >= 4) {
      print("API Response: $response");
      setState(() {
        uoclist = response; // Update the state with the new list
        print('ini respon klik :$uoclist');
      });
      print('State updated');
    }
    return uoclist; // Return the updated list
  }
            ...
            if (state is ValidateCaptchaSuccess) {
              if (state.response.status == "SUCCESS") {
                ...
                context.read<RegisterCubit>().registeringAccount(
                      ...
                      // '',
                      //lastName
                      _namaPerseoranganController.text.isNotEmpty
                          ? _namaPerseoranganController.text
                          : '',
                      //email
                      _emailController.text.isNotEmpty
                          ? _emailController.text
                      ...
                      selectedKota,
                      //nik
                      _nikController.text.isNotEmpty ? _nikController.text : '',
                      //npwp
                      '',
                      //password
                      ...
                      children: [
                        Column(
                          ...
                          children: <Widget>[
                            ...
                            Container(
                              ...
                              child: Row(
                                children: [
                                  Expanded(
                                    child: TextFormField(
                                      controller: _namaPerseoranganController,
                            ...
                            //           SnackBar(
                            //               content: Text(
                            //                   'Masukkan minimal 4 karakter untuk memulai')),
                            //         );
                            ...
                            //             .uOCCreateAccount(filter);
                            //         print(
                            //             "Response from uOCCreateAccount: $response");

                            //         // Check if response is null or empty
                            //         if (response == null || response.isEmpty) {
                            //           return []; // Return an empty list if response is invalid
                            //         }
                                  ...
                                  child: MaterialButton(
                                    ...
                                    onPressed: () async {
                                      ...
                                      if (password != confirmPassword) {
                                        ...
                                        // final emailResponse = await context
                                        //     .read<CheckEmailCubit>()
                                        //     .checkEmail(_emailController.text);
                                        // print('emailResponse nya: $emailResponse');
                                        // if (emailResponse.toString() ==
                                        //     "Email is available") {
                                        //   context
                                        //       .read<RegisterCubit>()
                                        //       .generateCaptcha(
