...
import 'detail_invoice_home_niaga.dart';
import 'package:flutter/services.dart';

import 'home_niaga.dart';

class MyInvoiceHomeNiagaPage extends StatefulWidget {
  const MyInvoiceHomeNiagaPage({Key? key}) : super(key: key);
  // final String niagaToken;
  // const MyInvoiceHomeNiagaPage({Key? key, required this.niagaToken}) : super(key: key);

  @override
  State<MyInvoiceHomeNiagaPage> createState() => _MyInvoiceHomeNiagaPageState();
}

// class _MyInvoiceHomeNiagaPageState extends State<MyInvoiceHomeNiagaPage> {
class _MyInvoiceHomeNiagaPageState extends State<MyInvoiceHomeNiagaPage>
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      "No. Invoice",
                      style:
                          TextStyle(color: Colors.black, fontFamily: 'Poppin'),
                    ),
                    SizedBox(height: 8),
                    Container(
                      decoration: BoxDecoration(
        ...
        body: BlocConsumer<InvoiceNiagaCubit, InvoiceNiagaState>(
          ...
          builder: (context, state) {
            ...
            return BlocConsumer<InvoiceGroupCubit, InvoiceGroupState>(
              ...
              if (state is DeleteSingleInvoiceSuccess) {
                _fetchDataForPage(currentPageIndex + 1);
                // Navigator.pop(context);
              }
            }, builder: (context, state) {
              if (state is MultipleInvoiceInProgress ||
                  state is DeleteSingleInvoiceInProgress) {
                return Center(
                  child: CircularProgressIndicator(
                    color: Colors.amber[600],
                  ),
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          Checkbox(
                                            ...
                                            //   setState(() {
                                            //     _isCombinedPayment = value ?? false;
                                            //   });
                                            // },
                                            onChanged: (value) {
                                              setState(() {
                                                              ...
                                                              .map((entry) => {
                                                                    "no_invoice":
                                                                        entry
                                                                            .key,
                                                                    "total_invoice":
                                                                        entry.value['totalInvoiceKey'] ??
                                                                            0,
                                                                    "no_order_boon":
                                                                        entry.value['noJobKey'] ??
                                                                            '',
                                                                    "no_order_online":
                                                                        entry.value['noOrderOnlineKey'] ??
                                                                            'NULL',
                                                                  })
                                                              .toList();
                                                      print(
  ...
  void performSearch() {
    ...
    // BlocProvider.of<InvoiceNiagaCubit>(context).searchinvoice(
    //   pageIndex: pageIndex,
  ...
  Widget _buildInvoiceList(List<InvoiceItemAccesses> invoices) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        // String totalInvoiceKey = invoiceItem.totalInvoice.toString();
          ...
          children: [
            if (_isCombinedPayment)
              Checkbox(
                ...
                //     };
                //     print('Updated _selectedInvoices: $_selectedInvoices');
                //   });
                // },
                //BERDASARKAN CABANG
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                color: ((invoiceItem.statusPayment == "not_paid" ||
                            invoiceItem.statusPayment == "paid") &&
                        jatuhTempoDate != null &&
                        jatuhTempoDate.isBefore(today))
                    ? Color.fromARGB(255, 245, 179, 201)
                    : Colors.white,
                border: Border.all(
                  color: Color.fromARGB(255, 184, 33, 22),
                  width: 2.0, // Border width
