...
import 'package:niaga_apps_mobile/screen-niaga/detail_resi_order_niaga.dart';
...
class DetailOrderNiagaPage extends StatefulWidget {
  final List<DetailHeaderAccesses> detail;
  const DetailOrderNiagaPage({Key? key, required this.detail})
      : super(key: key);

  @override
  State<DetailOrderNiagaPage> createState() => _DetailOrderNiagaPageState();
}

class _DetailOrderNiagaPageState extends State<DetailOrderNiagaPage>
  ...
  void initState() {
    ...
    BlocProvider.of<PencarianBarangCubit>(context).searchPencarianBarang(
      penerima: '',
      noResi: widget.detail.isNotEmpty ? widget.detail[0].resiNumber ?? '' : '',
  ...
  void _fetchDataForPage(int pageNumber) {
    ...
    BlocProvider.of<PencarianBarangCubit>(context)
        // .pencarianBarang(pageIndex: pageNumber);
          ...
          return SingleChildScrollView(
            child: Column(
              children: [
                          ...
                          return InkWell(
                            onTap: () {
                              Navigator.pushReplacement(context,
                                ...
                                return DetailResiOrderNiagaPage(
                                    stok: data);
                              }));
                            },
                            child: Container(
                ...
                //     pencarianBarangModellist.isNotEmpty)
                if (state is SearchPencarianBarangSuccess && _totalPages > 1)
                  Padding(
  ...
  void performSearch() {
    ...
    if (searchTextTanggal.isEmpty &&
      ...
      // filteredList = pencarianBarangModellist;
      Navigator.of(context)
          .pop(); // Close the dialog if no search terms are provided
      return;
    }
