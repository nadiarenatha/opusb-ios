...
class TrackingNiagaDTDCOSLServiceImpl implements TrackingNiagaDTDCOSLService {
  final log = getLogger('TrackingNiagaDTDCOSLServiceImpl');
  final Dio dio;
  ...
  @override
  Future<List<TrackingDtdCoslAccesses>> getTrackingDTDCOSL(String noPL) async {
    List<TrackingDtdCoslAccesses> listTrackingDTDCOSL = [];

    try {
      // Log before making the request
      log.i('Making API request to get tracking DTD COSL...');
      ...
      if (response.statusCode == 200) {
        ...
        final trackingData = TrackingDtdCoslAccesses.fromJson(response.data);

        // Add the deserialized data to the list
        listTrackingDTDCOSL.add(trackingData);
      } else {
