...
class SearchPackingServiceImpl implements SearchPackingService {
  final log = getLogger('SearchPackingServiceImpl');
  final Dio dio;
  ...
  Future<List<PackingNiagaAccesses>> searchPackingComplete(
    ...
    try {
      ...
      // log.i(
      //   'api/v1/packing-list/?owner_code=$ownerCode&page=$page&size=5&status=COMPLETE&search_no_pl=$noPL&no_container=$containerNo&port_asal=$asal&port_tujuan=$tujuan&nama_kapal=$vesselName',
      // );

      String url =
          'api/v1/packing-list/?email=$email&page=$page&size=5&status=COMPLETE&search_no_pl=$noPL&no_container=$containerNo&port_asal=$asal&port_tujuan=$tujuan&nama_kapal=$vesselName';

      if (ownerCode != null && ownerCode != "ONLINE") {
        url += '&owner_code=$ownerCode';
      }

      log.i('url Packing Complete nya: $url');
      ...
      log.i("Full Response Complete: ${response.toString()}");
      log.i("Response status code Complete: ${response.statusCode}");
...
//LOG
class LogNiagaServiceImpl22 implements LogNiagaService22 {
  final log = getLogger('LogNiagaServiceImpl22');
  final Dio dio;
  final FlutterSecureStorage storage;
  ...
  Future<String> getFullUrl(int? page,
      String? noPL,
      String? containerNo,
      String? asal,
      String? tujuan,
    ...
    String endpoint = 'api/v1/packing-list/?email=$email&page=$page&size=5&status=COMPLETE&search_no_pl=$noPL&no_container=$containerNo&port_asal=$asal&port_tujuan=$tujuan&nama_kapal=$vesselName';
    if (ownerCode != 'ONLINE') {
  ...
  Future<LogNiagaAccesses> logNiaga(int? page,
    ...
    try {
      final fullUrl = await getFullUrl(page, noPL, containerNo, asal, tujuan, vesselName);
      log.i('Full URL Log Packing Complete: $fullUrl');
