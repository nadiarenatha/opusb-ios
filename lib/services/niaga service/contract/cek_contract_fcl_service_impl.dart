...
import 'cek_contract_fcl_service.dart';
import 'package:intl/intl.dart';

class ContractFCLServiceImpl implements ContractFCLService {
  final log = getLogger('ContractFCLServiceImpl');
  final Dio dio;
  ...
  Future<List<ContractAccesses>> getContractFCL(String portAsal, String portTujuan,
    ...
    try {
      ...
      log.i(
          'api/v1/contract?owner_code=$ownerCode&jenis_pengiriman=FCL&port_asal=$portAsal&port_tujuan=$portTujuan&uoc_asal=$uocAsal&uoc_tujuan=$uocTujuan');
...
//LOG
class LogNiagaServiceImpl28 implements LogNiagaService28 {
  Future<String> getFullUrl(String portAsal, String portTujuan, String uocAsal,
      String uocTujuan) async {
    final ownerCode = await storage.read(
          key: AuthKey.ownerCode.toString(),
          aOptions: const AndroidOptions(encryptedSharedPreferences: true),
        ) ??
  ...
  final log = getLogger('LogNiagaServiceImpl28');
  final Dio dio;
  final FlutterSecureStorage storage;
  ...
  Future<LogNiagaAccesses> logNiaga(String portAsal, String portTujuan,
    ...
    try {
      final fullUrl = await getFullUrl(
          portAsal, portTujuan, uocAsal, uocTujuan);
